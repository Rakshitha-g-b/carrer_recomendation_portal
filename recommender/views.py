...
from .models import College, City, State, StreamTag # Ensure these models exist
...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print(f"DEBUG: Registration form is invalid with errors: {form.errors}") # Corrected f-string
    else:
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print(f"DEBUG: Login form is invalid with errors: {form.errors}") # Corrected f-string
    else:
...
def dashboard_view(request):
    ...
    context = {
        ...
        'STREAM_ENTRANCE_EXAMS': STREAM_ENTRANCE_EXAMS, # For template access
        'FINANCIAL_AID_INFO': FINANCIAL_AID_INFO,       # For template access
    }
    ...
    if request.method == 'POST':
        ...
        if recommendation_form.is_valid():
            ...
            if not sorted_streams or sorted_streams[0][1] < NO_RECOMMENDATION_THRESHOLD :
                ...
                # For entrance exams in multiple suggestions
                ...
                if recommendation_type not in SPECIAL_RECOMMENDATION_TYPES:
                     ...
                     context['relevant_entrance_exams'] = STREAM_ENTRANCE_EXAMS.get(recommendation_type, []) # For single
            ...
            print(f"DEBUG: RecommendationInputForm is invalid with errors: {recommendation_form.errors}") # Corrected f-string
            context['recommendation_type'] = None
            
    print(f"DEBUG: Final context keys for dashboard: {list(context.keys())}")
    if 'college_search_form' in context:
        print(f"DEBUG: type of college_search_form in context: {type(context['college_search_form'])}")
    else:
        print("DEBUG: college_search_form is NOT in context")
...
def college_search_view(request):
    ...
    if form.is_valid():
        ...
        if request.GET and form.errors: 
...
def load_cities(request):
    ...
    print(f"DEBUG: Returning cities for load_cities: {cities_data}")
